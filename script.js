// EventLink - Beautiful, Free RSVP Tool
...
class EventLink {
    ...
    createEvent(event) {
        ...
        const slotsText = document.getElementById('eventSlots').value.trim();
        const slots = slotsText ? slotsText.split('\n').filter(s => s.trim()) : this.getDefaultSlots(theme);
    ...
    getDefaultSlots(theme) {
        const defaults = {
            potluck: ['Main Dish', 'Side Dish', 'Salad', 'Dessert', 'Drinks', 'Appetizers'],
            meeting: ['Presentation', 'Notes', 'Refreshments', 'Materials', 'Setup'],
            party: ['Food', 'Drinks', 'Music', 'Games', 'Decorations', 'Photos']
        };
        return defaults[theme] || ['Item 1', 'Item 2', 'Item 3'];
    }
    ...
    submitRSVP(event) {
        ...
        if (existingRSVP) {
            ...
            existingRSVP.slot = selectedSlot;
    ...
    renderEvent() {
        ...
        if (this.currentEvent.slots.length > 0) {
            slotsList.innerHTML = this.currentEvent.slots.map(slot => {
                const taken = this.currentEvent.rsvps.filter(r => r.slot === slot && r.status === 'yes').length;
                return `
                    <label class="cursor-pointer">
                        <input type="radio" name="slot" value="${slot}" class="radio radio-primary hidden peer">
                        <div class="p-3 border rounded-lg text-center peer-checked:bg-primary peer-checked:text-white peer-checked:border-primary hover:bg-gray-50 transition-colors">
                            <div class="font-medium">${slot}</div>
                            <div class="text-xs opacity-70">${taken} signed up</div>
                        </div>
                    </label>
                `;
            }).join('');
    ...
    renderGuests() {
        ...
        guestsList.innerHTML = rsvps.map(rsvp => {
            ...
            return `
                <div class="flex items-center gap-4 p-4 bg-base-200 rounded-lg animate-slide-in">
                    ...
                    <div class="flex-1">
                        ...
                        ${rsvp.email ? `<div class="text-sm text-gray-600">${rsvp.email}</div>` : ''}
                        ${rsvp.slot ? `<div class="text-sm text-gray-600">Bringing: ${rsvp.slot}</div>` : ''}
                        ${rsvp.notes ? `<div class="text-sm text-gray-500 italic">${rsvp.notes}</div>` : ''}
                    </div>
                </div>
            `;
        }).join('');
    ...
    exportGuests() {
        const csv = [
            ['Name', 'Email', 'Status', 'Slot', 'Notes', 'RSVP Date'],
    ...
    showDemo() {
        ...
        const demoEvent = {
            ...
            theme: 'potluck',
            slots: ['Main Dish', 'Side Dish', 'Salad', 'Dessert', 'Drinks'],
            rsvps: [
                { id: '1', name: 'John Doe', status: 'yes', slot: 'Main Dish', createdAt: new Date().toISOString() },
                { id: '2', name: 'Jane Smith', status: 'yes', slot: 'Dessert', createdAt: new Date().toISOString() },
                { id: '3', name: 'Mike Johnson', status: 'maybe', slot: 'Drinks', createdAt: new Date().toISOString() }
            ],
            createdAt: new Date().toISOString(),
    ...
    confetti() {
        ...
        const interval = setInterval(function() {
            ...
            // Create particles in random colors
